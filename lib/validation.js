...
function validateRSAA(action) {
  var validCallAPIKeys = ['method', 'endpoint', 'query', 'body', 'headers', 'credentials', 'bailout', 'types', 'meta'];
  ...
  if (typeof query !== 'undefined' && !_lodashIsplainobject2['default'](query)) {
  ...
  if (typeof types === 'undefined') {
  ...
  } else if (!Array.isArray(types) || types.length !== 4) {
    validationErrors.push('[RSAA].types property must be an array of length 4');
  } else {
    ...
    if (typeof abortType !== 'string' && typeof abortType !== 'symbol' && !isValidTypeDescriptor(abortType)) {
      validationErrors.push('Invalid abort type');
    }
  }
